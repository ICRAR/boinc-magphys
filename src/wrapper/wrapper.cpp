...
int TASK::run(int argct, char** argvt) {
    ...
    fprintf(stdout, "%s wrapper: running %s (%s)\n",
...
int main(int argc, char** argv) {
    ...
    fprintf(stdout, "wrapper: starting\n");
    
    boinc_get_init_data(aid);
    fprintf(stdout, "starting to look for job file");
    retval = parse_job_file();
    if (retval) {
        fprintf(stdout, "can't parse job file: %d\n", retval);
        boinc_finish(retval);
    }
    fprintf(stdout, "after parsing job file");
