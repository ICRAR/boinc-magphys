...
def migrate_image_files(connection, image_bucket, file_bucket, s3helper):
    for file_name in glob.glob('/home/ec2-user/galaxyImages/*/*'):
        ...
        if galaxy is not None:
            if extension == '.fits':
                add_file_to_bucket1(file_bucket, get_key_fits(galaxy[GALAXY.c.name], galaxy[GALAXY.c.run_id], galaxy[GALAXY.c.galaxy_id]), file_name, s3helper)
            else:
                ...
                if file_name.endswith('_tn_colour_1.png'):
                    add_file_to_bucket1(image_bucket, get_thumbnail_colour_image_key(galaxy_key, 1), file_name, s3helper)
                elif file_name.endswith('_colour_1.png'):
                    add_file_to_bucket1(image_bucket, get_colour_image_key(galaxy_key, 1), file_name, s3helper)
                elif file_name.endswith('_colour_2.png'):
                    add_file_to_bucket1(image_bucket, get_colour_image_key(galaxy_key, 2), file_name, s3helper)
                elif file_name.endswith('_colour_3.png'):
                    add_file_to_bucket1(image_bucket, get_colour_image_key(galaxy_key, 3), file_name, s3helper)
                elif file_name.endswith('_colour_4.png'):
                    add_file_to_bucket1(image_bucket, get_colour_image_key(galaxy_key, 4), file_name, s3helper)
                elif file_name.endswith('_mu.png'):
                    add_file_to_bucket1(image_bucket, get_build_png_name(galaxy_key, 'mu'), file_name, s3helper)
                elif file_name.endswith('_m.png'):
                    add_file_to_bucket1(image_bucket, get_build_png_name(galaxy_key, 'm'), file_name, s3helper)
                elif file_name.endswith('_ldust.png'):
                    add_file_to_bucket1(image_bucket, get_build_png_name(galaxy_key, 'ldust'), file_name, s3helper)
                elif file_name.endswith('_sfr.png'):
                    add_file_to_bucket1(image_bucket, get_build_png_name(galaxy_key, 'sfr'), file_name, s3helper)
...
def add_file_to_bucket1(file_bucket, key, path_name, s3helper):
    if DRY_RUN:
        ...
        s3helper.add_file_to_bucket(file_bucket, key, path_name)


def migrate_hdf5_files(connection, file_bucket, s3helper):
    for galaxy in connection.execute(select([GALAXY])):
        ...
        try:
            ...
            if check_results(output, path_name):
                add_file_to_bucket1(file_bucket, get_key_hdf5(galaxy[GALAXY.c.name], galaxy[GALAXY.c.run_id], galaxy[GALAXY.c.galaxy_id]), path_name, s3helper)
            else:
...
def migrate_files(connection):
    ...
    s3helper = S3Helper()
    image_bucket = s3helper.get_bucket(get_galaxy_image_bucket())
    file_bucket = s3helper.get_bucket(get_files_bucket())

    migrate_image_files(connection, image_bucket, file_bucket, s3helper)
    migrate_hdf5_files(connection, file_bucket, s3helper)
